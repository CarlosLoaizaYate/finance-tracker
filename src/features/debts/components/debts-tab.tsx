...
// Cuota numbers are plain 1-based position among regular payments, no
// offset. A statement's own "No. de Cuota que se Cancela" refers to the
// upcoming "Periodo Liquidado" printed on that SAME document (verified
// directly by the client against their bank: cuota 83's due-before-Oct-15
// bill was paid Sep 16) — not the payment detailed in "Comportamiento de
// su Crédito en el Periodo Anterior" further down the same page, which
// describes the PRECEDING cycle. An earlier version of this code compared
// a statement's cuota number against that other, one-cycle-earlier
// payment, which looked like a consistent +1 offset but was really just
// comparing two misaligned fields — there's no missing first cuota.
...
function MortgageCard({ mortgage, onDelete }: { mortgage: Mortgage; onDelete: () => void }) {
  ...
  return (
    <div style={{ ...card, marginBottom: 16 }}>
      ...
      {/* Progress bar — current estimate (reflects real extra payments) */}
      <div style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
